...
export function DashboardHeader() {
  ...
  useEffect(() => {
    const storedUser = localStorage.getItem("user")
    console.log("hhh",storedUser)
    if (storedUser) {
      try {
        const parsedUser = JSON.parse(storedUser)

        // Extract username and email from nested user_info
        const username = parsedUser?.username || "Unknown User"
        const email = parsedUser?.email || "No email found"

        setUser({ username, email })
        console.log("Fetched user from localStorage:", { username, email })
      } catch (err) {
        console.error("Failed to parse user data:", err)
      }
    }
