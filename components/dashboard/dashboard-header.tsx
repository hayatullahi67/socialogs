    ...
    const unsubscribe = onAuthStateChanged(auth, async (firebaseUser) => {
      ...
      try {
        ...
        const username =
          ...
          (firebaseUser.email ? firebaseUser.email.split("@")[0] : "Unknown User")
        const email = firebaseUser.email || profile?.email || "No email found"
        const profileImage = profile?.profileImage || firebaseUser.photoURL || undefined

        setUser({ username, email, profileImage })
        console.log("Fetched user from Firebase:", { uid: firebaseUser.uid, username, email })
      } catch (err) {
            ...
            <DropdownMenuContent align="end" className="w-[280px] p-4">
              ...
              <div className="flex items-center gap-3 mb-4">
                ...
                <div className="flex-1">
                  <div className="font-semibold text-base">{user?.username || "Loading..."}</div>
                  <div className="text-sm text-gray-500">{user?.email || "No email found"}</div>
                </div>
              </div>
