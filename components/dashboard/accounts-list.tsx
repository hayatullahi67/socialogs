...
//     // const purchaseData = {
//     //   userUUID: props.userUUID || 'anonymous',
//     //   accountId: selectedAccount.id,
//     //   platform: selectedAccount.platform,
//     //   quantity: quantity,
//     //   pricePerLog: selectedAccount.price,
//     //   totalAmount: selectedAccount.price * quantity,
//     //   credentials: soldCredentials,
...
//     //   platform: selectedAccount.platform,
//     //   quantity: quantity,
//     //   amount: selectedAccount.price * quantity,
//     //   status: 'completed',
...
const PAYSTACK_PUBLIC_KEY = "pk_live_a8bf895813c5d37ac119a81737309f69891decda"
...
/* helper platform functions (kept minimal / same behavior) */
const getPlatformIcon = (platform: string) => {
...
const getPlatformGradient = (platform: string) => {
  const p = (platform || "").toLowerCase()
  if (p.includes("instagram") || p.includes("ig")) return "from-purple-500 via-pink-500 to-orange-400"
  if (p.includes("facebook") || p.includes("fb")) return "from-blue-600 to-blue-500"
  if (p.includes("tiktok") || p.includes("tik tok")) return "from-black via-red-500 to-blue-400"
  if (p.includes("x") || p.includes("twitter")) return "from-gray-900 to-gray-700"
  if (p.includes("linkedin")) return "from-blue-700 to-blue-600"
  if (p.includes("google") || p.includes("voice")) return "from-blue-500 via-red-500 to-yellow-500"
  if (p.includes("snapchat") || p.includes("snap")) return "from-yellow-400 to-yellow-300"
  if (p.includes("vpn")) return "from-green-600 to-green-500"
  return "from-gray-600 to-gray-500"
}
...
export function AccountsList({ selectedPlatform, userUUID }: AccountsListProps) {
  const [firestoreAccounts, setFirestoreAccounts] = useState<FirebaseAccount[]>([])
  const [loading, setLoading] = useState(true)
  const [selectedAccount, setSelectedAccount] = useState<FirebaseAccount | null>(null)
  const [isModalOpen, setIsModalOpen] = useState(false)
  ...
  useEffect(() => {
    ...
    async function fetchFirestoreAccounts() {
      try {
        ...
        const q = query(accountsRef)
        const snapshot = await getDocs(q)
        const accounts = snapshot.docs.map((d) => ({ id: d.id, ...(d.data() as any) })) as FirebaseAccount[]
        if (!mounted) return
        setFirestoreAccounts(accounts)
      } catch (error) {
        console.error("Error fetching from Firestore:", error)
  ...
  const filteredAccounts =
    selectedPlatform === "all"
      ? firestoreAccounts
      : firestoreAccounts.filter((acc) => getPlatformCategory(acc.platform || "") === selectedPlatform)
  ...
  const handleProceedToPurchase = () => {
    ...
    try {
      const bulkLogs = selectedAccount.bulkLogs || []
      const parsedLogs: LogItem[] = bulkLogs.map((logString) => {
        const usernameMatch = logString.match(/username:\s*([^\s:]+)/i)
        const passwordMatch = logString.match(/password:\s*([^\s:]+)/i)
        return {
          username: usernameMatch ? usernameMatch[1] : "",
          password: passwordMatch ? passwordMatch[1] : "",
        }
  ...
  const handleQuantityChange = (increment: boolean) => {
    if (increment) {
      if (quantity < availableLogs.length) setQuantity((p) => p + 1)
      else alert(`Maximum quantity available is ${availableLogs.length} logs`)
    } else {
  ...
  const handleCompletePurchase = () => {
    ...
    // resolve uid and email from prop or Firebase Auth
    const auth = getAuth()
    const resolvedUID = userUUID || auth.currentUser?.uid
    const resolvedEmail = auth.currentUser?.email || `${resolvedUID || "guest"}@no-email.local`

    if (!resolvedUID) {
      console.error("Missing account or user UUID")
    ...
    const purchaseAccount = selectedAccount
    const purchaseQuantity = quantity
    const purchaseLogs = availableLogs.slice(0, quantity)
    ...
    const handler = (window as any).PaystackPop?.setup({
      ...
      amount: amountInKobo,
      currency: "NGN",
      ref: `PS-${resolvedUID}-${Date.now()}`,
      metadata: {
        custom_fields: [
          { display_name: "User ID", variable_name: "user_id", value: resolvedUID },
          { display_name: "Account ID", variable_name: "account_id", value: purchaseAccount.id },
        ],
      },
      onClose: function () {
        setPurchasing(false)
      ...
      callback: function (response: any) {
        // after successful payment, process DB updates
        console.log("Payment successful! Reference: " + response.reference)
        setPurchasing(true)

        const processPurchase = async () => {
          try {
            const soldCredentials: LogItem[] = purchaseLogs
            const remainingBulkLogs = (purchaseAccount.bulkLogs || []).slice(purchaseQuantity)
            const newLogsCount = Math.max(0, (purchaseAccount.logs || 0) - purchaseQuantity)

            const mainDocRef = doc(db, "uploads", purchaseAccount.id)
            await updateDoc(mainDocRef, { logs: newLogsCount, bulkLogs: remainingBulkLogs })

            const purchaseData = {
              userUUID: resolvedUID,
              accountId: purchaseAccount.id,
              platform: purchaseAccount.platform,
              followers: purchaseAccount.followers,
              mailIncluded: purchaseAccount.mailIncluded,
              quantity: purchaseQuantity,
              pricePerLog: purchaseAccount.price,
            ...
            setFirestoreAccounts((prev) =>
              prev.map((acc) => (acc.id === purchaseAccount.id ? { ...acc, logs: newLogsCount, bulkLogs: remainingBulkLogs } : acc))
            )
            console.log("✅ Purchase completed successfully")
          } catch (err) {
            console.error("❌ Purchase error:", err)
            alert("Purchase processing failed. Please contact support with reference: " + response.reference)
          } finally {
            setPurchasing(false)
          }
        }
    ...
    if (!handler) {
      alert("Payment library not loaded. Try again in a moment.")
      return
    }
  ...
  return (
    <>
      <div>
        <h2 className="text-base md:text-xl font-bold mb-3 md:mb-4 text-gray-900">
          {selectedPlatform === "all" ? "All Boosted Accounts" : `${selectedPlatform} Boosted Accounts`}
        </h2>

        {filteredAccounts.length === 0 ? (
          <div className="text-center py-8 text-gray-500">No accounts available for this platform</div>
        ) : (
          <div className="space-y-2 md:space-y-3">
            {filteredAccounts.map((account) => {
              const Icon = getPlatformIcon(account.platform)
              ...
              return (
                <div key={account.id} className="bg-white rounded-lg md:rounded-xl p-3 md:p-4 shadow-sm flex flex-col md:flex-row md:items-center gap-3 md:gap-4">
                  <div className="flex items-center gap-2 md:gap-3 flex-1">
                    <div className={cn("w-8 h-8 md:w-10 md:h-10 rounded-lg flex items-center justify-center flex-shrink-0 bg-gradient-to-br", gradient)}>
                      <Icon className="w-5 h-5 md:w-6 md:h-6 text-white" />
                    </div>
                    <div className="flex-1 min-w-0">
                      <h3 className="font-medium text-gray-900 text-xs md:text-sm">
                  ...
                  <div className="flex items-center justify-between md:justify-end gap-2 md:gap-3">
                    <div className={cn("px-2 md:px-3 py-1 rounded-md text-xs font-semibold", stockColor === "yellow" && "bg-yellow-100 text-yellow-800", stockColor === "green" && "bg-green-100 text-green-800", stockColor === "red" && "bg-red-100 text-red-800")}>
                      {account.logs || 0}pcs
                    </div>
                    <div className="text-sm md:text-base font-semibold text-gray-900 min-w-[80px] md:min-w-[100px] text-right">
                      ₦{account.price.toLocaleString()}
                    </div>
                    <Button onClick={() => handleBuyClick(account)} className="min-w-[60px] md:min-w-[70px] rounded-full font-semibold text-xs md:text-sm h-8 md:h-9 bg-primary hover:bg-primary/90 text-white">
                      Buy
                    </Button>
                  </div>
                </div>
      ...
      {isModalOpen && selectedAccount && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black bg-opacity-50" onClick={() => { if (!purchasing) { setIsModalOpen(false); setIsPurchaseMode(false); setPurchaseSuccess(false) } }}>
          <div className="bg-white rounded-2xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
            <div className="p-6 space-y-6">
              <div className="flex items-start justify-between">
                <div className="flex items-center gap-4">
                  <div className={cn("w-16 h-16 rounded-2xl flex items-center justify-center bg-gradient-to-br shadow-lg", getPlatformGradient(selectedAccount.platform))}>
                    {(() => {
                      const Icon = getPlatformIcon(selectedAccount.platform)
                      return <Icon className="w-8 h-8 text-white" />
                    })()}
                  ...
                  <div>
                    <h2 className="text-2xl font-bold text-gray-900">{selectedAccount.platform} Account</h2>
                    <p className="text-gray-600 text-base mt-1">{purchaseSuccess ? "Purchase Complete!" : isPurchaseMode ? "Select Quantity" : "Complete account details"}</p>
                  </div>
                </div>

                <button onClick={() => { if (!purchasing) { setIsModalOpen(false); setIsPurchaseMode(false); setPurchaseSuccess(false) } }} className="text-gray-400 hover:text-gray-600 transition-colors" disabled={purchasing}>
                  <X className="w-6 h-6" />
                </button>
              </div>
              ...
              {purchaseSuccess && (
                <div className="space-y-4">
                  ...
                  <div className="bg-gradient-to-br from-slate-50 to-slate-100 rounded-xl p-5 border border-slate-200">
                    ...
                    <div className="bg-white rounded-lg p-4 max-h-64 overflow-y-auto space-y-2 border border-slate-300">
                      {purchasedCredentials.map((cred, index) => (
                        <div key={index} className="font-mono text-sm bg-green-50 px-3 py-2 rounded border border-green-200">
                          <div className="text-gray-700"><span className="font-semibold">Username:</span> {cred.username}</div>
                          <div className="text-gray-700"><span className="font-semibold">Password:</span> {cred.password}</div>
                        </div>
                      ))}
                    </div>
                  </div>

                  <Button onClick={() => setIsModalOpen(false)} className="w-full h-12 rounded-xl font-semibold bg-green-600 hover:bg-green-700 text-white">Close</Button>
                </div>
              )}
              ...
              {isPurchaseMode && !purchaseSuccess && (
                <div className="space-y-6">
                  <div className="bg-gradient-to-br from-blue-50 to-blue-100 rounded-xl p-6 border border-blue-200">
                    ...
                    <div className="flex items-center justify-center gap-6 mb-6">
                      <Button onClick={() => handleQuantityChange(false)} disabled={quantity <= 1 || purchasing} className="w-12 h-12 rounded-full bg-red-500 hover:bg-red-600 text-white disabled:bg-gray-300 disabled:cursor-not-allowed"><Minus className="w-6 h-6" /></Button>
                      <div className="text-center">
                        <div className="text-5xl font-bold text-blue-900">{quantity}</div>
                        <div className="text-sm text-blue-700 mt-1">log(s)</div>
                      </div>
                      <Button onClick={() => handleQuantityChange(true)} disabled={quantity >= availableLogs.length || purchasing} className="w-12 h-12 rounded-full bg-green-500 hover:bg-green-600 text-white disabled:bg-gray-300 disabled:cursor-not-allowed"><Plus className="w-6 h-6" /></Button>
                    </div>

                    <div className="bg-white rounded-lg p-4 space-y-2">
                      <div className="flex justify-between text-sm"><span className="text-gray-600">Price per log:</span><span className="font-semibold text-gray-900">₦{selectedAccount.price.toLocaleString()}</span></div>
                      <div className="flex justify-between text-sm"><span className="text-gray-600">Quantity:</span><span className="font-semibold text-gray-900">{quantity}</span></div>
                      <div className="flex justify-between text-sm"><span className="text-gray-600">Available:</span><span className="font-semibold text-green-600">{availableLogs.length} logs</span></div>
                      <div className="border-t pt-2 mt-2 flex justify-between text-lg"><span className="font-bold text-gray-900">Total:</span><span className="font-bold text-green-600">₦{totalPrice.toLocaleString()}</span></div>
                    </div>
                  </div>

                  <div className="flex gap-3">
                    <Button onClick={() => setIsPurchaseMode(false)} disabled={purchasing} className="flex-1 h-12 rounded-xl font-semibold bg-gray-200 hover:bg-gray-300 text-gray-800">Back</Button>
                    <Button onClick={handleCompletePurchase} disabled={purchasing || quantity < 1} className="flex-1 h-12 rounded-xl font-semibold bg-gradient-to-r from-blue-600 to-blue-500 hover:from-blue-700 hover:to-blue-600 text-white shadow-lg">{purchasing ? "Processing..." : `Pay ₦${totalPrice.toLocaleString()}`}</Button>
                  </div>
                </div>
              )}
              ...
              {!isPurchaseMode && !purchaseSuccess && (
                <>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div className="bg-gradient-to-br from-blue-50 to-blue-100 rounded-xl p-4 border border-blue-200">
                      <div className="flex items-center gap-2 text-blue-700 mb-2"><Users className="w-5 h-5" /><span className="text-sm font-semibold">Followers</span></div>
                      <p className="text-2xl font-bold text-blue-900">{selectedAccount.followers.toLocaleString()}+</p>
                    </div>

                    <div className="bg-gradient-to-br from-green-50 to-green-100 rounded-xl p-4 border border-green-200">
                      <div className="flex items-center gap-2 text-green-700 mb-2"><DollarSign className="w-5 h-5" /><span className="text-sm font-semibold">Price per log</span></div>
                      <p className="text-2xl font-bold text-green-900">₦{selectedAccount.price.toLocaleString()}</p>
                    </div>

                    <div className="bg-gradient-to-br from-purple-50 to-purple-100 rounded-xl p-4 border border-purple-200">
                      <div className="flex items-center gap-2 text-purple-700 mb-2"><Package className="w-5 h-5" /><span className="text-sm font-semibold">Available Stock</span></div>
                      <p className="text-2xl font-bold text-purple-900">{selectedAccount.logs || 0} pcs</p>
                    </div>

                    <div className="bg-gradient-to-br from-orange-50 to-orange-100 rounded-xl p-4 border border-orange-200">
                      <div className="flex items-center gap-2 text-orange-700 mb-2"><Mail className="w-5 h-5" /><span className="text-sm font-semibold">Email Access</span></div>
                      <p className="text-lg font-bold text-orange-900">{selectedAccount.mailIncluded ? "Included ✓" : "Not Included"}</p>
                    </div>
                  </div>
                  ...
                  <div className="flex gap-3 pt-4 border-t">
                    <Button onClick={() => setIsModalOpen(false)} className="flex-1 h-12 rounded-xl font-semibold bg-gray-200 hover:bg-gray-300 text-gray-800">Close</Button>
                    <Button onClick={handleProceedToPurchase} disabled={selectedAccount.logs === 0} className="flex-1 h-12 rounded-xl font-semibold bg-gradient-to-r from-primary to-primary/80 hover:from-primary/90 hover:to-primary/70 text-white shadow-lg disabled:opacity-50">Proceed to Purchase</Button>
                  </div>
                </>
              )}
            </div>
